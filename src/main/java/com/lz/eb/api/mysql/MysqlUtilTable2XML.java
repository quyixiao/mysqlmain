...
import com.lz.eb.api.MysqlMain;
import com.lz.eb.api.StringUtil;
...
public class MysqlUtilTable2XML {
    ...
    public static void printXMLForMap(TablesBean bean) {
        ...
        bf.append("<mapper namespace=\"" + MysqlMain.dao_package + "." + realName + "Dao\">").append("\n");
        bf.append("\n");
        bf.append("    <!-- 通用查询映射结果 -->\n");
        ...
        bf.append(TAB).append("<update id=\"updateCover" + bean.getSpaceName() + "ById\" parameterType=\"").append(MysqlMain.bean_package).append(".").append(bean.getSpaceName()).append("\" >").append("\n");
        bf.append(TAB).append(TAB).append("update").append("\n");
        ...
        String fileName = MysqlMain.save_path + "/" + realNam2 + "Dao.xml";

        if (StringUtil.isNotBlank(MysqlMain.mapper_xml_path)) {
            fileName = MysqlMain.mapper_xml_path + "/" + realNam2 + "Dao.xml";
        }
        ...
        try {
            ...
            System.out.println("===" + realName + "Dao.xml" + "生成");
